...
app.get("/get_nip_pegawai_khusus", function (req, res) {
	let sql = "SELECT nip_pegawai FROM pembagian_tugas WHERE nip_petugas = ?";
	conn.query(sql, [req.session.user.nip], function (err, result) {
		if (err) {
			console.error("Error fetching NIP pegawai:", err);
			return res.status(500).json({
				message: "Internal Server Error",
		...
		// Extract nip_pegawai values from the result into a new array
		let nipPegawaiArray = result.map((item) => item.nip_pegawai);

		res.json({
			message: "NIP Pegawai",
			status: 200,
			error: null,
			response: nipPegawaiArray, // Send the array of nip_pegawai values
		});
	});
});
...
app.post("/login", function (req, res) {
	...
	if (req.session && req.session.user) {
		...
		return redirectToDashboard(user.access_level, res, user.username);
	}
...
function redirectToDashboard(accessLevel, res, username) {
	let redirectUrl;
	switch (accessLevel) {
		case "pegawai":
...
app.post("/send_reset_password", (req, res) => {
	...
	const insertParams = [email, token, expiresAt];
...
app.post("/reset_password/:token", async (req, res) => {
	...
	const checkTokenQuery =
		"SELECT * FROM reset_tokens WHERE token = ? AND expires_at > NOW()";
	conn.query(checkTokenQuery, [token], async (err, result) => {
		if (err) {
			console.error("Error checking reset token:", err);
			return res
				.status(500)
				.json({ status: 500, error: "Internal Server Error", response: null });
		}
...
app.post("/assign_location", function (req, res) {
	const { nip_pegawai, latitude, longitude, assignment_date } = req.body;

	const sql =
		"INSERT INTO lokasi_penugasan (nip_pegawai, latitude, longitude, assignment_date) VALUES (?, ?, ?, ?)";
	const values = [nip_pegawai, latitude, longitude, assignment_date];
...
const officeLatitude = -7.274104;
const officeLongitude = 112.78797287;
const maxDistance = 50;

app.post("/cek_lokasi", function (req, res) {
	const { latitude, longitude } = req.body;
	const nipPegawai = req.session.user.nip;
	...
	const checkLocationSql =
		"SELECT * FROM lokasi_penugasan WHERE nip_pegawai = ? AND assignment_date = ?";
	conn.query(checkLocationSql, [nipPegawai, today], function (err, result) {
		if (err) {
			console.error("Error checking location assignment:", err);
			return res.status(500).json({
				message: "Internal Server Error",
				status: 500,
				error: err.message,
		...
		let assignedLatitude = officeLatitude;
		let assignedLongitude = officeLongitude;

		if (result.length > 0) {
			assignedLatitude = result[0].latitude;
			assignedLongitude = result[0].longitude;
			console.log(assignedLatitude);
			console.log(assignedLongitude);
		}

		// Calculate the distance between the assigned location and the employee's location
		...
		if (distance > maxDistance) {
			return res.status(400).json({
				message: "You are not in the assigned area",
				status: 400,
		...
		const checkSql =
			"SELECT * FROM log_absensi WHERE nip_pegawai = ? AND DATE(waktu_masuk) = ?";
		conn.query(checkSql, [nipPegawai, today], function (err, result) {
			if (err) {
				console.error("Error checking existing kehadiran:", err);
				return res.status(500).json({
					message: "Internal Server Error",
					status: 500,
					error: err.message,
			...
			if (result.length > 0) {
				return res.status(400).json({
					message: "You have already submitted kehadiran for today",
					status: 400,
			...
			// If the location is within the assigned area and no existing entry, return success
			res.json({
				message: "Verified",
				status: 200,
				error: null,
...
app.post("/cek_lokasi_only", function (req, res) {
	const { latitude, longitude } = req.body;
	const nipPegawai = req.session.user.nip;
...
app.post(
	"/entry_kehadiran",
	upload.single("foto_bukti_masuk"),
	function (req, res) {
		const { latitude, longitude } = req.body;
		const nipPegawai = req.session.user.nip;
		...
		const waktuMasuk = jakartaTime.toISOString();
		let insertSql = "INSERT INTO log_absensi SET ?";
		let data = {
			nip_pegawai: nipPegawai,
			waktu_masuk: waktuMasuk,
			foto_bukti_masuk: req.file.filename, // Assuming filename is stored in req.file
		};

		let lokasi_sql = "INSERT INTO lokasi SET ?";
		let data2 = {
			nip_pegawai: nipPegawai,
			waktu: waktuMasuk,
			latitude: latitude,
		...
		conn.query(insertSql, data, function (err, result) {
			if (err) {
			...
			conn.query(lokasi_sql, data2, function (err, result2) {
				if (err) {
					console.error("Error entering lokasi:", err);
					return res.status(500).json({
						message: "Internal Server Error",
						status: 500,
						error: err.message,
				...
				res.json({
					message: "Kehadiran and Lokasi entered",
					status: 200,
					error: null,
					response: { kehadiran: result, lokasi: result2 },
				});
			});
		});
	}
...
app.post(
	"/entry_pulang",
	upload.single("foto_bukti_pulang"),
	function (req, res) {
		...
		const foto_bukti_pulang = req.file.filename;
		...
		conn.query(
			sql,
			[waktu_pulang, foto_bukti_pulang, nipPegawai],
			function (err, result) {
				if (err) {
					console.error("Error entering kepulangan:", err);
					return res.status(500).json({
						message: "Internal Server Error",
						status: 500,
						error: err.message,
...
app.get("/history_absensi", function (req, res) {
	...
	let query = conn.query(sql, [req.query.nip], function (err, result) {
		if (err) {
