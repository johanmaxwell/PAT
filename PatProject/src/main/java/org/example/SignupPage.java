...
public class SignupPage extends JPanel {
    ...
    private void signUp() {
        ...
        if (username.isEmpty() || password.isEmpty() || nip.isEmpty() || email.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Harap Mengisi Seluruh Informasi!", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        try {
            // Create Petugas object
            Petugas petugas = new Petugas(username, password, nip, email);

            // Send Petugas object via HTTP POST request to Node.js API
            HttpUtil.signupPetugas(petugas);
